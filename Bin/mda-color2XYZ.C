...
main( int argc, char *argv[] )
{
  ...
  if( !parser.parse( index, argc, argv ) || index!= argc )
  {
    parser.usage( argv[0], "<options>\n"
		  "Convert from a given color space to XYZ\n" );
    exit( 1 );
  }
  ...
  errorCond( channels.vec.size()== space->getDimension(),
	     "  number of channels does not match color space dimension!" );
  
  // setup output MDA object
  MDAWriter writer;
  writer.connect( cout );
  if( !writer.writeHeader( dim, 3, type ) )
  {
  ...
  for( i= numScanlines ; i> 0 ; i-- )
  {
	...
	origSpace[k]= value;
      ...
      // convert one pixel
      space->toXYZ( origSpace, XYZ );
      
      // write to output scanline
      for( k= 0 ; k< 3 ; k++ )
	typeConvert( &XYZ[k], Double, 
